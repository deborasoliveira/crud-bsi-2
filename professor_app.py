...
def ver(): #CONSULTA DE TODOS OS USUÁRIOS
    rows = comando.listar()
    app.listaProfessor.delete(0, END)
    for r in rows:
        app.listaProfessor.insert(END, r)

def procurar(): #BUSCA DE USUÁRIO ESPECÍFICO
    app.listaProfessor.delete(0, END)
    rows = comando.buscar(app.txtNome.get(), app.txtcpf.get(), app.txtDEP.get()) #UTILIZAÇÃO DA FUNÇÃO DE "BUSCAR" DO BANCO E UTILIZANDO AS INFORMAÇÕES DAS VARIÁVEIS
    for r in rows:
        app.listaProfessor.insert(END, r)

def inserir():
    comando.cadastrar(app.txtNome.get(), app.txtcpf.get(), app.txtDEP.get()) #CADASTRANDO
    ver() #CONFERIR O CADASTRO

def update():
    comando.atualizacao(selected[0],app.txtNome.get(), app.txtcpf.get(), app.txtDEP.get()) #ALTERANDO O CADASTRO
...
def getSelectedRow(event): #PREENCHER OS CAMPOS DE ENTRADA COM AS INFORMAÇÕES DAS PESSOAS JA CADASTRADAS
    global selected
    index = app.listaProfessor.curselection()[0] #RECUPERAR O INDEX DOS ITENS SELECIONADOS, PRIMEIRA POSIÇÃO RETORNADA POR CURSERELATION
...
if __name__ == "__main__":
    app = Aplicacao()
    app.listaProfessor.bind('<<ListboxSelect>>', getSelectedRow) #ASSOCIANDO A FUNÇÃO GETSELECTED AO LISTBOX, SEMPRE QUE O ITEM FOR SELECIONADO, ESSA FUNÇÃO SERÁ ATIVADA.
